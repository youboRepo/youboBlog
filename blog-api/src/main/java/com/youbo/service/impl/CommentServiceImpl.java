...
import com.youbo.mapper.CommentMapper;
...
public class CommentServiceImpl implements CommentService
{
	...
	@Override
	public List<com.youbo.entity.Comment> getListByPageAndParentCommentId(Integer page, Long blogId, Long parentCommentId) {
		List<com.youbo.entity.Comment> comments = commentMapper.getListByPageAndParentCommentId(page, blogId, parentCommentId);
		for (com.youbo.entity.Comment c : comments) {
			//递归查询子评论及其子评论
			List<com.youbo.entity.Comment> replyComments = getListByPageAndParentCommentId(page, blogId, c.getId());
	...
	@Override
	public com.youbo.entity.Comment getCommentById(Long id) {
		com.youbo.entity.Comment comment = commentMapper.getCommentById(id);
		if (comment == null) {
	...
	public void updateCommentPublishedById(Long commentId, Boolean published) {
		...
		if (!published) {
			List<com.youbo.entity.Comment> comments = getAllReplyComments(commentId);
			for (com.youbo.entity.Comment c : comments) {
				hideComment(c);
			}
		}
	...
	@Transactional(rollbackFor = Exception.class)
	@Override
	public void updateComment(com.youbo.entity.Comment comment) {
	 ...
	 */
	private void delete(com.youbo.entity.Comment comment) {
		for (com.youbo.entity.Comment c : comment.getReplyComments()) {
			delete(c);
		}
	 ...
	 */
	private void hideComment(com.youbo.entity.Comment comment) {
		for (com.youbo.entity.Comment c : comment.getReplyComments()) {
			hideComment(c);
		}
	 ...
	 * @return
	 */
	private List<com.youbo.entity.Comment> getAllReplyComments(Long parentCommentId) {
		List<com.youbo.entity.Comment> comments = commentMapper.getListByParentCommentId(parentCommentId);
		for (com.youbo.entity.Comment c : comments) {
			List<com.youbo.entity.Comment> replyComments = getAllReplyComments(c.getId());
			c.setReplyComments(replyComments);
		}
		return comments;
	}
